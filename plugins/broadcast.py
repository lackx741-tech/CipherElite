...
def init(client_instance):
    ...
    """
    commands = [
        ".gcast <target> [copy] - Broadcast message to all chats/groups/users with V26 engine"
    ]
    description = "📡 V26 Broadcast System - Advanced message broadcasting with intelligent targeting"
    add_handler("broadcast", commands, description)  # Short, professional button name

async def register_commands():
    """
    V26 broadcast system with advanced targeting
    """
    
    class V26BroadcastEngine:
        def __init__(self):
    ...
    broadcast_engine = V26BroadcastEngine()
    
    @V26Userbot.on(events.NewMessage(pattern=r"\.gcast\s+(.+)"))
    @rishabh()
    async def v26_broadcast(event):
        try:
            if not event.reply_to_msg_id:
                await event.reply("🎭 **V26 Broadcast System**\n\n"
            ...
            if not args:
                await event.reply("❌ **V26 Error:** Please specify target (all/groups/users)")
                return
            
            target_type = args[0].lower()
            
            if target_type not in ["all", "groups", "users"]:
                await event.reply("🎭 **V26 Broadcast Error**\n\n"
                                "❌ **Invalid target type!**\n\n"
                                "**Valid targets:**\n"
                                "• `all` - All chats\n"
                                "• `groups` - Groups only\n"
            ...
            if not reply_message:
                await event.reply("❌ **V26 Error:** Could not get replied message!")
                return
            
            # Initial status message
            status_msg = await event.reply("🎭 **V26 Broadcast System**\n\n"
            ...
            result_msg = f"🎭 **V26 Broadcast Complete**\n\n"
    ...
    async def broadcast_stats(event):
        try:
            ...
            stats_msg = f"🎭 **V26 Broadcast Statistics**\n\n"
